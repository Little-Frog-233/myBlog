  ...
  <div class="comment-input" :id="'comment-content-input-' + randomId">
    <div>
      <input
        ...
        @focus="showCommentButtonFunc()"
        v-model:value="commentContent"
        :id="'comment-content-input-input-' + randomId"
      />
    </div>
    <div style="float: right;margin-top: 10px;" v-show="showCommentButton">
      <button
        style="background-color: #027fff;color: white;padding: .2rem 1.1rem;"
        :id="'comment-content-input-input-button-' + randomId"
        @click="postComment()"
      >评论</button>
    </div>
  </div>
...
<script>
const { checkUser, postComment, postReply } = require("../js/utils.js");
...
export default {
  ...
  created() {
    ...
    body.addEventListener(
      ...
      e => {
        if (
          e.target.id == "comment-content-input-" + this.randomId ||
          e.target.id == "comment-content-input-input-" + this.randomId ||
          e.target.id == "comment-content-button-" + this.randomId
        ) {
        } else {
          this.showCommentButton = false;
        }
